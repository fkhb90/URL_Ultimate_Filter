...
-------------------------
當前版本：V46.25 (2026-06-13)
最新架構更新：
- [Privacy] Perplexity 手機驗證狀態端點精準封鎖：`www.perplexity.ai` 新增 `/api/auth/phone-verification/status` 至 `CRITICAL_PATH_MAP`，不影響既有 `rest/sse` 事件流。
...
- [Privacy] Perplexity 手機驗證狀態端點精準封鎖：
  ...
  - 既有 www.perplexity.ai/rest/sse/entry_creation_events 維持放行，避免誤傷正常 SSE 事件流
"""
...
RULES_DB = {
    ...
    "BLOCK_DOMAINS": [
        ...
        'track.tiara.daum.net', 'stat.tiara.daum.net', 'track.tiara.kakao.com', 'vwo.com', 'yieldlab.net',
        ...
        'talkingdata.com', 'mdap.alipay.com', 'loggw-ex.alipay.com', 'dypnsapi-dualstack.aliyuncs.com',
    ...
    "BLOCK_DOMAINS_WILDCARDS": [
        ...
        'zztfly.com', 'qchannel01.cn', 'qchannel03.cn',
    ...
    "CRITICAL_PATH_MAP": {
        ...
        'www.perplexity.ai': ['/api/auth/phone-verification/status'],
    ...
    "PATH_EXEMPTIONS": {
        ...
        "x.com": ["/i/api/graphql/", "/account/authenticate_web_view"]
    }
}
...
def generate_full_coverage_cases() -> List[TestCase]:
    ...
    # --- V45.41 Naver Maps 廣告圖片 CDN + NTM 標籤管理器封鎖 ---
    # Apollo State 逆向分析：RestaurantAdSummary.adImages → searchad-phinf.pstatic.net
    ...
    # --- V45.95 qchannel03.cn ---
    cases.append(TestCase("Privacy: qchannel03 Tracking Block", "https://i.qchannel03.cn/combine?akey=gdios-16.16.6&u=enc", RES_BLOCK_403, "V45.95 qchannel03.cn 中國渠道歸因追蹤封鎖；akey=gdios 為 iOS 平台標識，u= 加密用戶 ID；BLOCK_DOMAINS_WILDCARDS 同 qchannel01.cn"))
    ...
    # --- V46.25 Perplexity precise phone verification status block ---
    cases.append(TestCase("Privacy: Perplexity Phone Verification Status Block", "https://www.perplexity.ai/api/auth/phone-verification/status", RES_BLOCK_403, "V46.25 www.perplexity.ai 手機驗證狀態查詢端點以 CRITICAL_PATH_MAP 精準封鎖；不擴大到其他 auth/API 路徑"))
    cases.append(TestCase("Safe: Perplexity SSE Entry Creation Events Pass", "https://www.perplexity.ai/rest/sse/entry_creation_events", RES_ALLOW, "V46.25 保留 www.perplexity.ai SSE 事件流放行；確認精準 phone-verification/status 規則不誤傷 rest/sse 功能"))
    ...
    # --- V45.68 dypnsapi-dualstack + acs.m.taobao ---
    ...
    cases.append(TestCase("Privacy: Resend Dead-Clicks Block", "https://resend.com/relay-qWzP/static/dead-clicks-autocapture.js?v=1.372.9", RES_BLOCK_403, "V46.02 resend.com 郵件收件人點擊行為追蹤腳本封鎖；.js+/static/ 雙重旁路導致 pathScanner SKIP，CRITICAL_PATH_MAP dead-clicks-autocapture Step 5 精準攔截"))
